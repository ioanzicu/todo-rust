...
impl JwToken {
    pub fn get_key() -> String {
        ...
        let key_str: &str = config.map.get("SECRET_KEY").unwrap().as_str().unwrap();

        key_str.to_owned()
    }

    pub fn encode(self) -> String {
        let key: EncodingKey = EncodingKey::from_secret(JwToken::get_key().as_ref());
        let token: String = encode(&Header::default(), &self, &key).unwrap();
    ...
    pub fn from_token(token: String) -> Result<Self, String> {
        let key: DecodingKey = DecodingKey::from_secret(JwToken::get_key().as_ref());
...
mod jwt_tests {
    ...
    async fn test_passing_token_request() {
        ...
        let header_name = header::HeaderName::from_str("token").unwrap();
        let header_value = header::HeaderValue::from_str(encoded_token.as_str()).unwrap();
    ...
    async fn test_false_token_request() {
        ...
        let header_value = header::HeaderValue::from_str("test").unwrap();
        req.headers_mut().insert(header_name, header_value);
        let resp = call_service(&app, req).await;
        assert_eq!("401", resp.status().as_str());
    }
